...
describe("win/lose conditions read the way their labels do", () => {
  it("a fresh match is not already over", () => {
    ...
    expect(GameLogic.isConditionMet(START, detective.loseCondition)).toBe(
      false,
    );
    expect(GameLogic.isConditionMet(START, vigilante.winCondition)).toBe(false);
    expect(GameLogic.isConditionMet(START, vigilante.loseCondition)).toBe(
      false,
    );
  });
  ...
  it("Detective wins only when both green stats reach 100", () => {
    expect(
      GameLogic.isConditionMet(
        { ...START, investigation: 100 },
        detective.winCondition,
      ),
    ).toBe(false);
    expect(
      GameLogic.isConditionMet(
    ...
    expect(
      GameLogic.isConditionMet(
        { ...START, investigation: 100, morale: 100 },
        detective.winCondition,
      ),
    ).toBe(true);
  });
  ...
  it("Detective loses when any red stat reaches 100", () => {
    expect(
      GameLogic.isConditionMet(
        { ...START, publicOpinion: 100 },
        detective.loseCondition,
      ),
    ).toBe(true);
    expect(
      GameLogic.isConditionMet(
  ...
  it("Vigilante wins only when both green stats reach 100", () => {
    ...
    expect(
      GameLogic.isConditionMet(
        { ...START, morale: 100, publicOpinion: 100 },
        vigilante.winCondition,
      ),
    ).toBe(true);
  });
